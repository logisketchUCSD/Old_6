	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			if (numArgs == 0)
			{
				...
				Console.WriteLine("***  -i <input stream>: Input stream of values in the proper Assign3 specification.");
			...
			if (argArray.Contains("-i"))
			{
				...
				// Doing this just to test with Keller's code easier
				x_array.Reverse();
				d_array.Reverse();
		...
		/*		ArrayList x_array = new ArrayList();
				...
				layer1.Add(new Adeline2(new ArrayList(new double[]{-0.25, -0.48}), FunctionType.Logistic));
				layer1.Add(new Adeline2(new ArrayList(new double[]{-0.41, -0.13}), FunctionType.Logistic));

				layer2.Add(new Adeline2(new ArrayList(new double[]{0.09, -0.17, 0.48}), FunctionType.Linear));

				layers.Add(layer1);
				layers.Add(layer2);

				NeuralNets.BackProp bpNetwork = new NeuralNets.BackProp(layers, x_array, d_array);

				bpNetwork.Train(0.10, 1);  */
				...
				layer1.Add(new Adeline2(new ArrayList(
						new double[]{RandomClass.NextDouble() * 0.1, RandomClass.NextDouble() * 0.1}), 
					FunctionType.Logistic));
				layer1.Add(new Adeline2(new ArrayList(
						new double[]{RandomClass.NextDouble() * 0.1, RandomClass.NextDouble() * 0.1}), 
					FunctionType.Logistic));

				layer2.Add(new Adeline2(new ArrayList(
						new double[]{RandomClass.NextDouble() * 0.1, RandomClass.NextDouble() * 0.1, 
									 RandomClass.NextDouble() * 0.1}), 
					FunctionType.Linear));
